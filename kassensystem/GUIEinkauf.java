...
public class GUIEinkauf extends JFrame {
	...
	/*
	 * Zur Übersichtlichkeit und Wartungsfreundlichkeit: Falls sich die Reihenfolge
	 * der Spalten nachträglich ändern sollte, muss dies nur hier und nicht im
	 * gesamten Code korrigiert werden.
	 * 
	 * Die Endpreisspalte ist immer die letzte Spalte, nur bei einkausListe(-Model)
	 * verwendbar!!!!
	 */
	int NameSpalte = 0;
	int EANSpalte = 1;
	...
	// Bildschirmauflösung des Nutzers ermitteln um das Fenster und die Elemente
	// dynamisch anpassen
	...
	/**
	 * Diese Funktion aktualisiert die Zwischensumme, welche in der Variable zValue
	 * gespeichert wird, um den Wert "Input". Anschließend wird der Wert noch auf 2
	 * Nachkommastellen gerundet.
	 * 
	 * @param Input ein Float, der den zu verändernden Betrag der Zwischensumme
	 *              enthält
	 */
	...
	/**
	 * Die Funktion ermittelt die Ware Zeile des asugewählten Artikels. Dies wird
	 * benötigt, um nach dem Filtern der Liste auch den korrekten Index der
	 * Artikelzeile zu erhalten, da sonst bei einer gefilterten Liste der Index der
	 * gefilterten Liste genutzt wird, um den Artikel in der ungefilterten Liste zu
	 * identifizieren.
	 * 
	...
	/**
	 * Mit dem veränderten Bestand (effektiv die Einkausliste) wird die xml auf den
	 * aktuellen Stand gebracht. Also alle Artikel die auf der Einkaufsliste sind,
	 * werden nach und nach von der xml Datei in Menge oder Stückzahl (kommt auf den
	 * Artikel an) reduziert.
	 * 
	 * @param daten enthält die Daten, die abgespeichert werden sollen, findet hier
	 *              nur Verwendung mit dem Einkaufslistenmodell.
	 */
	 ...
	 * 
	 * @param data Variable, die Anwendung im Zusammenhang mit bestandsListeModel
	 *             findet
	 */
	...
	public GUIEinkauf() {
		...
		String[] columnNames = { "Artikelname", "EAN", "Stückpreis", "Stückzahl", "Grundpreis", "Grundpreiseinheit",
				"Menge", "Mengeneinheit", "Kategorie" };

		Object[][] dataBestand = {};
		Object[][] dataEinkauf = {};

		// Hier wird die Einkaufsliste inkl. Scrollbalken und dazugehörigem Modell
		// definiert
		...
		einkaufsListe
				.setPreferredScrollableViewportSize(new Dimension((int) screenWidth / 3, (int) screenHeight - 200));
		...
		// Hier sind alle Elemente des zentralen Abschluss-Interaktionspanels, dieses
		// wird erst beim Einkaufsabschluss sichtbar
		...
		// Hier wird die Bestandsliste inkl. Scrollbalken und dazugehörigem Modell
		// definiert und mit Daten gefüllt
		...
		// Hier wird für die Einkaufsliste eine zusätzliche Spalte hinzugefügt, die
		// immer die den "Endpreis", also den Preis, mit dem das Produkt gemeinsam mit
		// seiner Menge preislich gewichtet wird.
		...
		/**
		 * Hier wird der "addButton" mit seinen Funktionen definiert. Dieser überträgt
		 * bei Benutzung die ausgewählte Zeile der Bestandsliste in die Einkaufsliste.
		 * Anfangs wird außerdem direkt abgefangen, dass der Benutzer nicht mehr Artikel
		 * übertragen kann, als vorhanden sind.
		 * 
		 * Handelt es sich um ein Produkt dessen Menge in "Stück" angegeben wird, wird
		 * pro Mausklick je ein Produkt übernommen und einzeln in der Einkaufsliste
		 * aufgeführt.
		 * 
		 * Handelt es sich um ein Produkt dessen Menge nicht in Stück angegeben ist, so
		 * wird ein extra Textfeld mit Button eingeblendet ("insertValue" und
		 * "addValueButton" zum bestätigen). Mit diesen kann dann eine konkrete Menge
		 * durch den Nutzer angegeben werden. Je nach Produkt und Preis-/Mengeneinheit
		 * wird entsprechend umgerechnet (um den Faktor "Mult"). Dieser wird zur
		 * Berechnung des endgültigen Preises für das Produkt und der Zwischensumme
		 * verwendet.
		 */
			...
			public void actionPerformed(ActionEvent e) {
				if (!bestandsListeModel.getValueAt(getRow(bestandsListe), StkZahl).equals("n")) {
					if (Float.parseFloat((String) bestandsListeModel.getValueAt(getRow(bestandsListe), StkZahl)) > 0) {
						bestandsListeModel.setValueAt(
								String.valueOf(Float.parseFloat(
										(String) bestandsListeModel.getValueAt(getRow(bestandsListe), StkZahl)) - 1),
								getRow(bestandsListe), StkZahl);
						Vector neuerArtikelAufEinkaufsListe = new Vector(
								bestandsListeModel.getDataVector().elementAt(getRow(bestandsListe)));
						...
						einkaufsListeModel.setValueAt(
								(String) bestandsListeModel.getValueAt(getRow(bestandsListe), StkPreis),
								einkaufsListeModel.getRowCount() - 1, EndpreisSpalte);
						changeZwischensumme(Float
								.parseFloat((String) bestandsListeModel.getValueAt(getRow(bestandsListe), StkPreis)));
					}
				}

				else {
					if (Float.parseFloat(
							(String) bestandsListeModel.getValueAt(getRow(bestandsListe), MengeSpalte)) > 0) {
						...
						addValueButton.addActionListener(new ActionListener() {
							public void actionPerformed(ActionEvent e) {
								if (Float
										.parseFloat(((String) bestandsListeModel.getValueAt(getRow(bestandsListe),
												MengeSpalte))) >= Float.parseFloat(insertValue.getText())
										&& Float.parseFloat(insertValue.getText()) > 0) {
									bestandsListeModel.setValueAt(
											String.valueOf(Float
													.parseFloat((String) bestandsListeModel
															.getValueAt(getRow(bestandsListe), MengeSpalte))
													- Float.parseFloat(insertValue.getText())),
											getRow(bestandsListe), MengeSpalte);
									Vector neuerArtikelAufEinkaufsListe = new Vector(
											bestandsListeModel.getDataVector().elementAt(getRow(bestandsListe)));
									einkaufsListeModel.addRow(neuerArtikelAufEinkaufsListe);
									einkaufsListeModel.setValueAt(String.valueOf(insertValue.getText()),
											einkaufsListeModel.getRowCount() - 1, MengeSpalte);
									einkaufsListeModel.setValueAt(
											String.valueOf(Float.parseFloat(insertValue.getText())
													* Float.parseFloat(((String) einkaufsListeModel.getValueAt(
															einkaufsListeModel.getRowCount() - 1, GrundPreis)))),
											einkaufsListeModel.getRowCount() - 1, EndpreisSpalte);
									changeZwischensumme(Float.parseFloat((String) einkaufsListeModel
											.getValueAt(einkaufsListeModel.getRowCount() - 1, EndpreisSpalte)));
								}
		...
		/**
		 * Dieser MouseListener löscht den Inhalt des Eingabefeldes "insertValue",
		 * sobald dieses mit der Maus angeklickt wird. Dies dient vorallem dazu die
		 * Bedienerfreundlichkeit der Funktion unter dem "addValueButton" zu erhöhen.
		 * Beachte: Dieses wird nur angezeigt, wenn ein Produkt zur Einkaufsliste
		 * hinzugefügt werden soll, dessen Einheit nicht in "Stück" angegeben ist.
		 * (siehe auch "addButton"-Funktion)
		 */
		...
		/**
		 * Dieser MouseListener löscht den Inhalt des Eingabefeldes "searchField",
		 * sobald dieses mit der Maus angeklickt wird. Dies dient vorallem dazu die
		 * Bedienerfreundlichkeit der Funktion unter dem "searchButton" zu erhöhen.
		 * 
		 */
		...
		/**
		 * Dieser MouseListener löscht den Inhalt des Eingabefeldes "BarGeldField",
		 * sobald dieses mit der Maus angeklickt wird. Dies dient vorallem dazu die
		 * Bedienerfreundlichkeit der Funktion unter dem "WechelgeldBerechnenButton" zu
		 * erhöhen. Beachte: Dieses Feld mit dem dazugehörigen Button wird erst nach dem
		 * Abschließen des Einkaufs sichtbar.
		 * 
		 */
		...
		/**
		 * Dieser Actionlistener löscht einzelne Elemente aus der Einkaufsliste. Es wird
		 * immer das Element gelöscht das vom Nutzer ausgewählt wurde. Dieser geht die
		 * gesamte Bestandsliste durch, bis das, im Bestand, zu reduzierende Produkt
		 * gefunden worden ist, dieses wird dann um den entsprechenden Betrag
		 * verringert. Pro Stück jeweils um den Wert 1, bei nicht-Stück Mengen um den
		 * jeweiligen Betrag, um den das Produkt vorher hinzugefügt wurde.
		 */
		removeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!bestandsListeModel.getValueAt(i, StkZahl).equals("n")) {
					bestandsListeModel.setValueAt(
							String.valueOf(Float.parseFloat((String) bestandsListeModel.getValueAt(i, StkZahl)) + 1), i,
							StkZahl);
				} else {
					bestandsListeModel.setValueAt(String.valueOf(
							Float.parseFloat((String) einkaufsListeModel.getValueAt(getRow(einkaufsListe), MengeSpalte))
									+ Float.parseFloat((String) bestandsListeModel.getValueAt(i, MengeSpalte))),
							i, MengeSpalte);
				}

				changeZwischensumme(Float
						.parseFloat((String) (einkaufsListeModel.getValueAt(getRow(einkaufsListe), EndpreisSpalte)))
						- 2 * Float.parseFloat(
								(String) (einkaufsListeModel.getValueAt(getRow(einkaufsListe), EndpreisSpalte))));
				einkaufsListeModel.removeRow(getRow(einkaufsListe));
			}
		});

		/**
		 * Funktioniert im Kern wie die Funktionalität des "removeButtons". Es wird
		 * zusätzlich für jedes Element über die gesamte Einkaufsliste iteriert, um
		 * jedes Element zu entfernen. Jedoch werden nur die Artikel auf der
		 * Einkaufsliste wieder der Bestandsliste hinzugefügt. Zur Vereinfachung wird
		 * die Zwischensumme am Ende auf "Zwischensumme: 0€" gesetzt und die
		 * Einkaufsliste komplett gelöscht.
		 */
			...
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < (Integer) einkaufsListe.getRowCount(); i++) {
					...
					if (!einkaufsListeModel.getValueAt(i, StkZahl).equals("n")) {
						bestandsListeModel.setValueAt(
								String.valueOf(
										Float.parseFloat((String) bestandsListeModel.getValueAt(j, StkZahl)) + 1),
								j, StkZahl);
					} else {
						bestandsListeModel.setValueAt(
								String.valueOf(Float.parseFloat((String) einkaufsListeModel.getValueAt(i, MengeSpalte))
										+ Float.parseFloat((String) bestandsListeModel.getValueAt(j, MengeSpalte))),
								j, MengeSpalte);
					}
				}
		...
		/**
		 * Hiermit wird der Einkauf abgeschlossen. Mit dem veränderten Bestand (effektiv
		 * die Einkausliste) wird die xml auf den aktuellen Stand gebracht. Also alle
		 * Artikel die auf der Einkaufsliste sind, werden nach und nach von der xml
		 * Datei in Menge oder Stückzahl (kommt auf den Artikel an) reduziert.
		 * 
		...
		/**
		 * Mit diesem Button gelangt Nutzer zur Abschlussansicht des Einkaufs. Dort wird
		 * dem die Endsumme und das zu gebende Wechselgeld angezeigt, wenn der Nutzer
		 * erhaltenes Bargeld angibt.
		 */
		...
		/**
		 * Berechnet nach dem Betätigen des Buttons das Wechselgeld. Wechselgeld wird
		 * gemeinsam mit der Endsumme ausgegeben.
		 * 
		 * Beachte: Dies ist für den Nutzer erst sichtbar, wenn dieser im
		 * Einkaufsabschluss ist.
		 */
		WechelgeldBerechnenButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ErrorArea.setText("");
					AbschlussArea.setText("Endsumme: " + zValue + "\n Wechselgeld: "
							+ (Float.parseFloat(BarGeldField.getText()) - zValue) + " €");
		...
		/**
		 * Durch Änderungen, jeglicher Art, im Suchfeld "searchField" wird die Suche
		 * durchgeführt. Mittels eines RowFilters wird die Liste nach der Eingabe im
		 * "searchField" gefiltert. D.h., alle Artikel werden angezeigt, die den
		 * Suchtext enthalten. Ist keine Suche eingegeben, wird alles ungefiltert
		 * ausgegeben.
		 */
